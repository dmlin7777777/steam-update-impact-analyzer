...
class GPUOptimizedFeatureEngineer:
            ...
            try:
                ...
                with torch.no_grad():  # 节省GPU内存
                    for i in range(0, len(df), batch_size):
                        batch_texts = df['review_content_clean'].iloc[i:i+batch_size].fillna('').astype(str).tolist()
                        
                        # 限制文本长度以避免GPU内存不足
                        batch_texts = [text[:512] if isinstance(text, str) and len(text) > 512 else str(text) for text in batch_texts]
    ...
    def extract_tfidf_features(self, df: pd.DataFrame, max_features: int = 1000) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        # 使用处理后的文本进行TF-IDF
        # 确保所有文本都是字符串类型
        texts = df['review_content_processed'].fillna('').astype(str).tolist()
        ...
        for fname in os.listdir(combined_dir):
            ...
            try:
                if lower.endswith('.csv'):
                    ...
                    df = None
                    for encoding in ['utf-8', 'gbk', 'gb2312', 'latin-1', 'iso-8859-1', 'cp1252']:
                        try:
                            df = pd.read_csv(path, encoding=encoding, nrows=max_rows_per_file)
                            logger.debug(f"Successfully read {fname} with encoding: {encoding}")
                            break
                        except (UnicodeDecodeError, UnicodeError):
                            continue
                    if df is None:
                        logger.warning(f"Could not decode CSV {path} with any common encoding, skipping")
                        continue
    ...
    def process_and_save_news_level(self, reviews_dir: str, news_dir: str, output_path: str,
        ...
        try:
            ...
            # 可能缺少 pyarrow / fastparquet 等依赖，回退到 CSV
            logger.warning(f"Could not write parquet to {output_path}: {e}. Falling back to CSV.")
            csv_path = os.path.splitext(output_path)[0] + '.csv'
            news_level.to_csv(csv_path, index=False, encoding='utf-8')
            logger.info(f'News-level aggregation saved to CSV fallback: {csv_path} (rows: {len(news_level)})')
        return news_level
    ...
    def save_features(self, results: Dict[str, any], output_dir: str, file_prefix: str = "features", include_flagged: Optional[bool] = None):
        ...
        # 保存主要DataFrame
        df_path = os.path.join(output_dir, f"{file_prefix}{flag_suffix}_dataframe.xlsx")
        results['dataframe'].to_excel(df_path, index=False)
...
def main():
    """GPU 优化的示例使用方法 — 按 data/combined 下的每个文件（genre）逐个处理并保存特征。"""
    # 检查 GPU 可用性
    ...
    reviews_dir = r"C:\Users\12932\Desktop\nus\BAP\test data\reviews"
    ...
    for fname in os.listdir(reviews_dir):
        ...
        try:
            if lower.endswith('.csv'):
                ...
                for encoding in ['utf-8', 'gbk', 'gb2312', 'latin-1', 'iso-8859-1', 'cp1252']:
                    try:
                        df_file = pd.read_csv(path, encoding=encoding)
                        logger.info(f"Successfully read {fname} with encoding: {encoding}")
                        break
                    except (UnicodeDecodeError, UnicodeError):
                        continue
                if df_file is None:
        ...
        # 推断 genre 名称（与 load_reviews_from_combined_dir 保持一致）
        m = re.search(r'combined[_\-]?(.+?)[_\-]?reviews', fname, flags=re.IGNORECASE)
        if m:
            genre = m.group(1).lower()
        else:
        ...
        # 采样以控制单次处理规模（和之前 main 的行为一致）
        sample_size = min(2000, len(df_file)) if force_cpu else min(10000, len(df_file))
        ...
        # save per-genre
        feature_engineer.save_features(results, out_dir, prefix, include_flagged=True)
